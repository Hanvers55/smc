...
export type JournalEntry = {
  ...
  score: number; // 0-100
  checked: string[]; // rule ids that were checked (kosong untuk entri AI)
  total: number;
  note: string;
  createdAt: string; // ISO date
  source?: "manual" | "ai";
  result?: TradeResult | null; // diisi belakangan setelah trade selesai
};
...
export function setJournalEntryResult(
  entries: JournalEntry[],
  id: string,
  result: TradeResult | null
): JournalEntry[] {
  const next = entries.map((e) => (e.id === id ? { ...e, result } : e));
  saveJournal(next);
  return next;
}
